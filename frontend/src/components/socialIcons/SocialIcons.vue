<template>
  <div class="showArticleFooter__icons">
    <svg
      width="172"
      height="20"
      viewBox="0 0 172 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      xmlns:xlink="http://www.w3.org/1999/xlink"
    >
      <mask
        id="mask0_48_1662"
        style="mask-type: alpha"
        maskUnits="userSpaceOnUse"
        x="0"
        y="0"
        width="20"
        height="20"
      >
        <rect width="20" height="20" fill="url(#pattern0)" />
      </mask>
      <g mask="url(#mask0_48_1662)">
        <rect width="20" height="20" fill="#E5E5E5" />
      </g>
      <mask
        id="mask1_48_1662"
        style="mask-type: alpha"
        maskUnits="userSpaceOnUse"
        x="38"
        y="0"
        width="20"
        height="20"
      >
        <rect x="38" width="20" height="20" fill="url(#pattern1)" />
      </mask>
      <g mask="url(#mask1_48_1662)">
        <rect x="38" width="20" height="20" fill="#E5E5E5" />
      </g>
      <mask
        id="mask2_48_1662"
        style="mask-type: alpha"
        maskUnits="userSpaceOnUse"
        x="76"
        y="0"
        width="20"
        height="20"
      >
        <rect x="76" width="20" height="20" fill="url(#pattern2)" />
      </mask>
      <g mask="url(#mask2_48_1662)">
        <rect x="76" width="20" height="20" fill="#E5E5E5" />
      </g>
      <mask
        id="mask3_48_1662"
        style="mask-type: alpha"
        maskUnits="userSpaceOnUse"
        x="114"
        y="0"
        width="20"
        height="20"
      >
        <rect x="114" width="20" height="20" fill="url(#pattern3)" />
      </mask>
      <g mask="url(#mask3_48_1662)">
        <rect x="114" width="20" height="20" fill="#E5E5E5" />
      </g>
      <mask
        id="mask4_48_1662"
        style="mask-type: alpha"
        maskUnits="userSpaceOnUse"
        x="152"
        y="0"
        width="20"
        height="20"
      >
        <rect x="152" width="20" height="20" fill="url(#pattern4)" />
      </mask>
      <g mask="url(#mask4_48_1662)">
        <rect x="152" width="20" height="20" fill="#E5E5E5" />
      </g>
      <defs>
        <pattern
          id="pattern0"
          patternContentUnits="objectBoundingBox"
          width="1"
          height="1"
        >
          <use xlink:href="#image0_48_1662" transform="scale(0.0333333)" />
        </pattern>
        <pattern
          id="pattern1"
          patternContentUnits="objectBoundingBox"
          width="1"
          height="1"
        >
          <use xlink:href="#image1_48_1662" transform="scale(0.0333333)" />
        </pattern>
        <pattern
          id="pattern2"
          patternContentUnits="objectBoundingBox"
          width="1"
          height="1"
        >
          <use xlink:href="#image2_48_1662" transform="scale(0.0333333)" />
        </pattern>
        <pattern
          id="pattern3"
          patternContentUnits="objectBoundingBox"
          width="1"
          height="1"
        >
          <use xlink:href="#image3_48_1662" transform="scale(0.0333333)" />
        </pattern>
        <pattern
          id="pattern4"
          patternContentUnits="objectBoundingBox"
          width="1"
          height="1"
        >
          <use xlink:href="#image4_48_1662" transform="scale(0.0333333)" />
        </pattern>
        <image
          id="image0_48_1662"
          width="30"
          height="30"
          xlink:href="data:image/png;base64,"
        />
        <image
          id="image1_48_1662"
          width="30"
          height="30"
          xlink:href="data:image/png;base64,"
        />
        <image
          id="image2_48_1662"
          width="30"
          height="30"
          xlink:href="data:image/png;base64,"
        />
        <image
          id="image3_48_1662"
          width="30"
          height="30"
          xlink:href="data:image/png;base64,"
        />
        <image
          id="image4_48_1662"
          width="30"
          height="30"
          xlink:href="data:image/png;base64,"
        />
      </defs>
    </svg>
  </div>
</template>
